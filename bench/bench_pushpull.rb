...
# PUSH/PULL 2-process throughput benchmark with backend comparison.
#
# Spawns separate push (sender) and pull (receiver) processes per cell,
# measures throughput and sender CPU, writes JSONL to ~/.cache/omq/ruby/.
#
# Usage:
#   ruby --yjit bench/bench_pushpull.rb                         # auto-detect backends
#   ruby --yjit bench/bench_pushpull.rb --backends ruby,rust    # specific backends
#   ruby --yjit bench/bench_pushpull.rb --quick                 # 3 sizes, 1 round
#   ruby --yjit bench/bench_pushpull.rb --chart                 # generate SVG after
...
JSONL_PATH = File.join(CACHE_DIR, "results_pushpull.jsonl")
...
def run_cell(backend, msg_size, duration)
  ...
  Timeout.timeout(CELL_TIMEOUT) do
    push_pid, push_out = spawn_peer("push", "tcp://127.0.0.1:0", msg_size, backend: backend)
    ...
    pull_pid, pull_out = spawn_peer("pull", "tcp://127.0.0.1:#{port}", msg_size, duration.to_s, backend: backend)
    output = pull_out.read.strip
    pull_out.close
    pull_out = nil
    Process.wait(pull_pid)
    pull_pid = nil

    cpu_time = ChartHelper.read_proc_cpu(push_pid)
    push_out.close
    push_out = nil
    kill_peer(push_pid)
    push_pid = nil
  ...
  push_out&.close
  pull_out&.close
  kill_peer(push_pid) if push_pid
  kill_peer(pull_pid) if pull_pid
end
...
def main
  ...
  puts "PUSH/PULL | 2-process TCP loopback | Ruby #{RUBY_VERSION} (#{jit})"
  puts "run_id: #{run_id}"
  puts
  ...
  if opts[:chart]
    system(RUBY_BIN, File.join(__dir__, "gen_pushpull_chart.rb"))
  end
end
