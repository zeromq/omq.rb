...
# Generate PUSH/PULL backend comparison chart from JSONL results.
#
# Reads:  ~/.cache/omq/ruby/results_pushpull.jsonl
# Writes: doc/charts/pushpull/backends_tcp.svg
#
# Usage: ruby bench/gen_pushpull_chart.rb
...
OUTPUT     = File.join(REPO_ROOT, "doc", "charts", "pushpull", "backends_tcp.svg")
...
def load_data
  unless File.exist?(JSONL_PATH)
    $stderr.puts "ERROR: #{JSONL_PATH} not found. Run bench_pushpull.rb first."
    exit 1
  end
  ...
  rows = File.readlines(JSONL_PATH).filter_map do |line|
    ...
    row if row[:pattern] == "pushpull"
  end

  if rows.empty?
    $stderr.puts "ERROR: no pushpull rows in #{JSONL_PATH}"
    exit 1
  end
...
def main
  ...
  svg = ChartHelper.generate_single_panel_svg(
    title:        "PUSH/PULL throughput: TCP loopback, 2-process (#{backends_label})",
    hw_label:     hw,
    sizes:        sizes,
    series:       series,
    colors:       COLORS,
